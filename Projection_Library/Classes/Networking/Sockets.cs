...
namespace RedCorona.Net
{
    ...
    public class ClientInfo
    {
        ...
        {
            sock = cl; buffer = string.Empty; OnReadBytes = readevt; encType = encryptionType;
                ...
                switch (MessageType)
                {
                    case MessageType.CodeAndLength:
                        ...
                        if (encType != EncryptionType.None)
                            Send(new byte[] { checksum }); //Send Checksum
                        break;
    ...
    public class Sockets
    {
        ...
        public static Socket CreateTCPSocket(IPAddress address, int port) { return CreateTCPSocket(address, port, UseSocks, SocksProxy); }
        public static Socket CreateTCPSocket(IPAddress address, int port, bool useSocks, SocksProxy proxy)
        {
            Socket sock;
            
            if (useSocks)
                sock = ConnectToSocksProxy(proxy.host, proxy.port, address.ToString(), port, proxy.username, proxy.password);
            else
            {
#if NET_1_1
				IPAddress host = Dns.GetHostByName(address).AddressList[0];
